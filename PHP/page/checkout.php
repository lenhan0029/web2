                                ...
                                <table>
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($_SESSION['cart'] as $product) {
                                            ...
                                            $discounting += checkProductIsDiscounting($product['idpr']) ? $product['price'] * (100 - $discounting_today['percent']) / 100 : 0;
                                            ?>
                                            <tr id="tr<?php echo $product['idpr'] ?>">
                                                <input type="text" hidden value=<?php echo $product['price'] ?> id="price<?php echo $product['idpr'] ?>">
                                                <input hidden type="text" name="checkout[<?php echo $product['idpr'] ?>][qty]" value="<?php echo $product['qty'] ?>" />
                                                <td class="product-remove"><a style="cursor:pointer" onclick="delete_cart(<?php echo $product['idpr'] ?>)">×</a></td>
                                                <td class="product-thumbnail"><a href="http://localhost:8080/ProjectPHP/PHP/?page=product&id=<?php echo $product['idpr'] ?>"><img style="width:100px !important;height:120px !important;max-width:100px !important  " src="public/img/product/<?php echo $product['image'] ?>" alt=""></a></td>
                                                <td class="product-name"><a href="http://localhost:8080/ProjectPHP/PHP/?page=product&id=<?php echo $product['idpr'] ?>"><?php echo $product['namepr'] ?></a></td>
                                                <td class="product-name"><?php $size = getSizebyid($product['size'])->fetch_assoc(); echo $size['size'] ?></td>
                                                <td class="product-name"><span class="amount"><?php echo number_format($product['price']) ?></span></td>
                                                <td class="product-name"><input class="form-control" style="width:80px" onclick="update_cart(<?php echo $product['idpr'] ?>,<?php echo $product['size'] ?>,this)" onchange="update_cart(<?php echo $product['idpr'] ?>,<?php echo $product['size'] ?>,this)" min=1 type="number" value="<?php echo $product['qty'] ?>" /></td>
                                                <td class="product-name"><a href="#" id="total<?php echo $product['idpr'] ?>"><?php echo number_format($product['qty'] * $product['price']) ?>VND</a></td>
                                                <td class="product-stock-status"><span class="wishlist-in-stock"><?php if (checkProductIsDiscounting($product['idpr'])) echo $discounting_today['percent'];
